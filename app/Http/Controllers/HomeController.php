...
class HomeController extends Controller
{
    ...
    public function load_more_product(Request $request){
        ...
        if(!$all_product->isEmpty()){
            ...
            foreach($all_product as $key => $pro){
                ...
                <input type="hidden" id="wishlist_productprice'.$pro->id.'" value="'.number_format($pro->price,0,',','.').'VNĐ">
    ...
    $product = Product::whereIn('category_id',$sub_array)->where('status','1')->orderBy('id','DESC')->take(6)->get();
        ...
        foreach($product as $key => $val) {
            ...
            $output.='
              ...
              <input type="hidden" value="'.$val->id.'" class="cart_product_id_'.$val->id.'">
                ...
                <input type="hidden" value="'.$val->price.'" class="cart_product_price_'.$val->id.'">
